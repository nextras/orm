...
COMMIT;
SELECT "tags".* FROM "tags" AS "tags" WHERE "tags"."id" = 1;
SELECT "tag_id", COUNT(DISTINCT "count") as "count" FROM (SELECT "tag_followers"."tag_id", "tag_followers"."author_id" AS "count" FROM "tag_followers" AS "tag_followers" WHERE "tag_followers"."tag_id" IN (1)) AS "temp" GROUP BY "tag_id";
